...
class _AhadethTabState extends State<AhadethTab> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          SizedBox(height: 20), // مساحة من أعلى
          // تحديد حجم الصورة عشان ميحصلش overflow
          SizedBox(
            height: 80, // ارتفاع محدد للوجو
            child: Image.asset(
          ...
          SizedBox(height: 20),
          // استخدام Flexible بدلاً من Expanded مع تحديد constraints
                  ...
                  return SizedBox(
                    ...
                    child: CarouselSlider.builder(
                      ...
                      options: CarouselOptions(
                        ...
                        height: constraints.maxHeight - 40, // ترك مساحة للحواف
                        scrollDirection: Axis.horizontal,
                        enlargeFactor: .1,
                        viewportFraction: 0.8, // تحديد عرض العنصر
                      ),
  ...
  Widget buildHadethWidget(int itemIndex) {
    return InkWell(
      ...
      child: Container(
        margin: EdgeInsets.symmetric(horizontal: 8), // مساحة بين العناصر
        padding: EdgeInsets.symmetric(vertical: 20, horizontal: 24),
        decoration: BoxDecoration(
          image: DecorationImage(
            image: AssetImage(AppAssets.hadethBg),
            fit: BoxFit.fill, // ضمان امتلاء الصورة للحاوي
          ),
        ),
        child: Column(
          ...
          children: [
            SizedBox(height: 16),
            // تحديد مساحة للعنوان مع حماية من overflow
            Container(
              constraints: BoxConstraints(maxHeight: 60), // حد أقصى للارتفاع
              child: Text(
                ahadeth[itemIndex].title,
                style: AppTextStyel.blackBold24,
                textAlign: TextAlign.center,
                overflow: TextOverflow.ellipsis, // قطع النص إذا كان طويل
                maxLines: 2, // حد أقصى سطرين
              ),
            ),
            SizedBox(height: 16),
            // باقي المساحة للمحتوى مع ضمان عدم overflow
            Expanded(
              child: Container(
                child: SingleChildScrollView(
                  physics: BouncingScrollPhysics(), // حركة أفضل للتمرير
                  child: Column(
                    children: [
                      Text(
                      ...
                      SizedBox(height: 20), // مساحة في النهاية
                    ],
                  ),
                ),
              ),
  ...
  void readAhadethFile() async {
    try {
      ...
      // يمكن إضافة معالجة أفضل للخطأ هنا
    }
  }
}
